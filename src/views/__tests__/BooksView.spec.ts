...
describe('BooksView', () => {
  ...
  it('zeigt den eingeloggten Benutzernamen an', async () => {
  ...
  it('löscht ein Buch nach Bestätigung und lädt die Liste neu', async () => {
    ...
    const deleteCall = fetchMock.mock.calls.find(
      ([url, options]) =>
        (url as string).includes(`/books/${buch1.id}`) && options?.method === 'DELETE',
    )
  ...
  it('schaltet den Favoriten-Status eines Buchs per PUT-Request um', async () => {
    ...
    const putCall = fetchMock.mock.calls.find(
      ([url, options]) =>
        (url as string).includes(`/books/${buch1.id}`) && options?.method === 'PUT',
    )
  ...
  it('filtert Bücher nach dem ausgewählten Regal in der Sidebar', async () => {
    ...
    const aktuellEintrag = listItems.find((item) => item.text().includes('Aktuell'))
    await aktuellEintrag!.trigger('click')
  ...
  it('leitet bei Logout zur Login-Seite weiter', async () => {
    global.fetch = createFetchMock() as unknown as typeof fetch
